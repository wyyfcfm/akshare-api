...
function displayReportData(data, reportType) {
    ...
    reportData.forEach(row => {
        ...
        headers.forEach(header => {
            ...
            // 数字类型判断
            if (typeof value === 'string' && (value.includes('亿') || value.includes('万') || value.includes('%'))) {
                cellClass = 'number';
                // 判断正负
                if (value.includes('-')) {
                    cellClass += ' negative';
...
async function loadMindmaps() {
    ...
    try {
        ...
        // 生成思维导图卡片
        mindmapList.innerHTML = data.mindmaps.map(mindmap => `
...
async function viewMindmap(filename) {
    ...
    mindmapContent.innerHTML = '<div class="loading"><div class="spinner"></div><p>加载思维导图...</p></div>';
    ...
    try {
        ...
        // 重新初始化 mermaid 图表
        mermaid.init(undefined, mindmapContent.querySelectorAll('.language-mermaid'));
